...
/**
* @Author: Jiehang CAO
* @Description: this is client model as service
...
//                    System.out.println("500ms keep");
    ...
    private void handleMessage(String message) {
        ...
        switch (command) {
            case COM_GROUP:
                ...
                for (int i = 0; i < userData.size(); i++) {
                    ...
                    if (recorder.contains(user.getUserName())) {
                        user.setNotify(true);
        ...
        System.out.println("message from sever: " + message + " end of message");
    }
    ...
    public boolean CheckLogin(String username, String IP, String password, StringBuffer buf) {
        ...
        Map<Integer, Object> map; // use to store messages to server
        try {
            //this is for many times trying connect
    ...
    public boolean CheckRegister(String username,String IP, String password, String email, StringBuffer buf) {
        Map<Integer, Object> map;// use to store message to server
        try {
            //this is for many times trying connect
            if (client == null || client.isClosed()) {
                client = new Socket(IP, port);
