...
/**
* @Author: Jiehang CAO
* @Description: server thread acting as worker
...
public class BackEndServer extends Thread {
    ...
    /**
     * handle client's messages and do some operations
     * @param readLine
     */
        ...
        private void sendMessageToClient() {
            ...
            if (onlineUser != null)
                while ((message = onlineUser.getMsg()) != null) {
                    ...
                    System.out.println(onlineUser.getUsername() + "data sending message: " + message + "remaining size" + onlineUser.getSession().size());
                }

    }

    /**
     * user's logout event : sever send message to all client
     * and set the users' status to offline
     */
    private void logOut() {
        if (onlineUser == null)
            return;
    ...
    /**
     * broadcast method is used to send message to allOnlineUsers
     * three conditions: 1) chat in group, 2) user log_out, 3)user sign_up
     * @param message
     * @param type
     */
    ...
    /**
     * this method is to get the users in group
     * @return
     */
    ...
    /**
     * this method is to get formatDate
